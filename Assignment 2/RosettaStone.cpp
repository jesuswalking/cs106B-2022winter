...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    if (kGramLength <= 0) error("kGramLength cant be negative or zero.");
    if (kGramLength > str.length()) return map;
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    // check if the map contains only zero or contain negative numbers.
    bool allzero = true;
    double sum = 0;
    for (string key : input) {
        if (input[key] != 0) allzero = false;
        if (input[key] < 0) allzero = false;
        sum += input[key] * input[key];
    }
    if (allzero) error("all the values in the map are zeros");

    // do the normal work.
    Map<string, double> nomalMap;
    for (string key : input) {
        nomalMap.put(key, input[key] / sqrt(sum));
    }
    return nomalMap;
}
